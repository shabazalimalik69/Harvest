...
app.get("/projects",async (req,res)=>{
  ...
  if(project_name){
   task= await projectsModel.find({project_name}).skip((page-1)*limit).limit(limit).sort({[sortBy]:order==="asc"?1:-1});
  }
  else{
     task= await projectsModel.find().skip((page-1)*limit).limit(limit).sort({[sortBy]:order==="asc"?1:-1});
...
app.post("/projects",async (req, res) => {
    ...
    try {
      ...
      if (task) {
        ...
        let newTask = await projectsModel.create({...req.body,id:`${Date.now()}:${project_name}:${Math.random()*999999}${project_code}${Math.random()}`});
        res.send(newTask);
      }
    } catch (e) {
  ...
  app.patch("/projects/:id",async (req,res)=>{
    let { id }=req.params;
    let projectUpdateOne = await projectsModel.updateOne({id},{$set:req.body});
